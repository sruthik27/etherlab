...
PCAP_PATH="$CAPTURE_DIR/protocol-mismatch-$TIMESTAMP.pcap"
DECODE_PATH="$CAPTURE_DIR/protocol-mismatch-$TIMESTAMP.txt"
...
cleanup() {
  local rc=$?
  set +e
  scenario_log "Destroying protocol-mismatch lab"
...
scenario_log "Ensuring the protocol-mismatch lab is down"
ensure_scenario_down "$TOPOLOGY"

scenario_log "Deploying the protocol-mismatch lab"
run_clab deploy -t "$TOPOLOGY"

lag_wait_for_convergence 30

scenario_log "Starting capture on sw1 eth1 for the protocol mismatch"
...
grep -q "Po1(SD)" "$SW1_STATE" || { scenario_log "Suspended Port-channel state missing on sw1"; exit 1; }
grep -q "Po1(SD)" "$SW2_STATE" || { scenario_log "Suspended Port-channel state missing on sw2"; exit 1; }
grep -q "PAgP" "$SW2_STATE" || { scenario_log "PAgP mismatch signature missing on sw2"; exit 1; }
[ -s "$PCAP_PATH" ] || { scenario_log "Protocol-mismatch pcap was not written"; exit 1; }
assert_decoded_pcap_has_packets "$DECODE_PATH" || { scenario_log "Protocol-mismatch pcap decode did not contain packets"; exit 1; }

scenario_log "Protocol-mismatch capture completed successfully"
